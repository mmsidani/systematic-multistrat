module SA2.Common.Table_market_data
...
let get_maxChangeId_market_data_changes ( dbContext : MultistrategySchema.Sa2_multistrategy ) =
...
let private updateCore_market_data fieldId sourceId divisor dateInstIdValues = 
    ...
    let changeId = get_maxChangeId_market_data_changes multistrategyContext |> ref
    ...
    for instId in uniqueInstIds do
                ...
                |> Array.map (         
                            fun ( d , i , v ) -> 
                                ...
                                if v - row.Value.Value = 0.0 && row.Divisor = divisor then
                                    ...
                                    Some ( new MultistrategySchema.Tbl_market_data( InstrumentId = i , FieldId = fieldId , SourceId = sourceId , Value = Nullable<float> v , Date = (d |> date2Obj ), Divisor = divisor ) ,                                     
                                        row )
                            )
        ...
        let newRecords =
            ...
            iDateInstIdValues 
                ...
                |> Array.map ( 
                    fun ( d , i , v ) -> 
                        new MultistrategySchema.Tbl_market_data( InstrumentId = i , FieldId = fieldId , SourceId = sourceId , Value = Nullable<float> v , Date = (d |> date2Obj ), Divisor = divisor ) )                          
        ...
        multistrategyContext.Tbl_market_data.InsertAllOnSubmit( newRecords )
...
let updateExisting_market_data sourceName sa2FieldName divisor dateTickerValues =
    ...
    dateTickerValues |> Array.map ( fun ( d , t , v ) -> ( d , Map.find t instIds , v ) ) |> updateCore_market_data fieldId sourceId divisor




let update_market_data sourceName sa2FieldName field divisor dateDescrTickerValues =
    ...
    dateDescrTickerValues |> Array.map ( fun ( d , _ , t , v ) -> ( d , Map.find t instIds , v ) ) |> updateCore_market_data fieldId sourceId divisor
    
    
    
    
let private getCore_market_data sa2FieldName tickers numRecords date =
...
let get_market_data sa2FieldName tickers =

    let mktData = getCore_market_data sa2FieldName tickers -1 DateTime.MinValue
...
let getSa2Symbol_market_data sa2FieldName symbols =

    let symbol2Ticker = symbols |> Set.ofList |> getTickerFromSymbol_instruments 
    let ticker2Symbol = symbol2Ticker |> Map.toList |> List.map ( fun ( s , t ) -> ( t , s ) ) |> Map.ofList
    let tickers =  ticker2Symbol |> keySet |> Set.toList

    let data = get_market_data sa2FieldName tickers

    data |> Map.map ( fun _ m -> m |> Map.toList |> List.map ( fun ( t , l ) -> ( Map.find t ticker2Symbol , l ) ) |> Map.ofList )




let getForDate_market_data numRecords date sa2FieldName tickers =

    let mktData = getCore_market_data sa2FieldName tickers numRecords date
